...
private:
    ...
    void listenForData(std::shared_ptr<boost::asio::ip::tcp::socket> socket)
    {
        ...
        boost::asio::async_read(*socket, boost::asio::buffer(*buffer),
                                [this, socket, buffer](boost::system::error_code ec, std::size_t bytesRead)
                                {
                                    if (!ec)
                                    {
                                        std::string header(buffer->data(), 4);
                                        std::cout << "[LOG] Received header: " << header << " (" << bytesRead << " bytes)" << std::endl;
        ...
        socket->async_read_some(boost::asio::buffer(*buffer),
                                [this, socket, buffer](boost::system::error_code ec, std::size_t length)
                                {
                                    if (!ec)
                                    {
                                        ...
                                        std::cout << "[LOG] Received text: " << message << " (" << length
                                                  << " bytes)" << std::endl;
                                    ...
                                    {
                                        std::cerr << "[ERROR] Receiving text failed: " << ec.message()
                                                  << std::endl;
                                    }
                                });
    }
    ...
    void receiveFile(std::shared_ptr<boost::asio::ip::tcp::socket> socket)
    {
        try
        {
            ...
            std::ofstream outputFile("received_file.mp4", std::ios::binary);
            if (!outputFile)
            {
            ...
            while (totalReceived < fileSize)
            {
                ...
                std::cout << "[LOG] Received packet: " << bytesRead << " bytes (Total: " << totalReceived
